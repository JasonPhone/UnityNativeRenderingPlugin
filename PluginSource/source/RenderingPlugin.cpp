...
// --------------------------------------------------------------------------
// SetMeshBuffersFromUnity, an example function we export which is called by one
// of the scripts.
...
extern "C" void UNITY_INTERFACE_EXPORT UNITY_INTERFACE_API
SetMeshBuffersFromUnity(void *vertexBufferHandle, int vertexCount,
                        float *sourceVertices, float *sourceNormals,
                        float *sourceUV) {
  // A script calls this at initialization time; just remember the pointer here.
  // Will update buffer data each frame from the plugin rendering event (buffer
  // update needs to happen on the rendering thread).
  g_VertexBufferHandle = vertexBufferHandle;
  g_VertexBufferVertexCount = vertexCount;

  // The script also passes original source mesh data. The reason is that the
  // vertex buffer we'll be modifying will be marked as "dynamic", and on many
  // platforms this means we can only write into it, but not read its previous
  // contents. In this example we're not creating meshes from scratch, but are
  // just altering original mesh data -- so remember it. The script just passes
  // pointers to regular C# array contents.
...
#if UNITY_WEBGL
typedef void(UNITY_INTERFACE_API *PluginLoadFunc)(
    IUnityInterfaces *unityInterfaces);
typedef void(UNITY_INTERFACE_API *PluginUnloadFunc)();

extern "C" void UnityRegisterRenderingPlugin(PluginLoadFunc loadPlugin,
                                             PluginUnloadFunc unloadPlugin);
...
static void DrawColoredTriangle() {
  ...
  float worldMatrix[16] = {
      cosPhi, -sinPhi, 0, 0, sinPhi, cosPhi, 0,          0,
      0,      0,       1, 0, 0,      0,      finalDepth, 1,
  };
...
static void ModifyTexturePixels() {
  ...
  int textureRowPitch;
  void *textureDataPtr = s_CurrentAPI->BeginModifyTexture(
      textureHandle, width, height, &textureRowPitch);
  if (!textureDataPtr)
    return;
  ...
  for (int y = 0; y < height; ++y) {
    ...
    for (int x = 0; x < width; ++x) {
      ...
      int vv = int((127.0f + (127.0f * sinf(x / 7.0f + t))) +
                   (127.0f + (127.0f * sinf(y / 5.0f - t))) +
                   (127.0f + (127.0f * sinf((x + y) / 6.0f - t))) +
                   (127.0f +
                    (127.0f * sinf(sqrtf(float(x * x + y * y)) / 4.0f - t)))) /
               4;
  ...
  s_CurrentAPI->EndModifyTexture(textureHandle, width, height, textureRowPitch,
                                 textureDataPtr);
}
...
static void ModifyVertexBuffer() {
  ...
  size_t bufferSize;
  void *bufferDataPtr =
      s_CurrentAPI->BeginModifyVertexBuffer(bufferHandle, &bufferSize);
  if (!bufferDataPtr)
    return;
  int vertexStride = int(bufferSize / vertexCount);

  // Unity should return us a buffer that is the size of `vertexCount *
  // sizeof(MeshVertex)` If that's not the case then we should quit to avoid
  // unexpected results. This can happen if
  // https://docs.unity3d.com/ScriptReference/Mesh.GetNativeVertexBufferPtr.html
  // returns a pointer to a buffer with an unexpected layout.
  ...
  for (int i = 0; i < vertexCount; ++i) {
    ...
    dst.pos[0] = src.pos[0];
    dst.pos[1] = src.pos[1] + sinf(src.pos[0] * 1.1f + t) * 0.4f +
                 sinf(src.pos[2] * 0.9f - t) * 0.3f;
    dst.pos[2] = src.pos[2];
...
static void drawToPluginTexture() { s_CurrentAPI->drawToPluginTexture(); }

static void drawToRenderTexture() { s_CurrentAPI->drawToRenderTexture(); }
